...
export default function PdfSummarizer() {
  ...
  const [fullTextResult, setFullTextResult] = useState<FullTextResult | null>(
    null
  );
      ...
      <CardContent>
        ...
        <div className="flex justify-end gap-2 mt-4">
          <Button
            ...
            disabled={(!dataUri && !url) || status === "loading"}
            className="w-full"
          >
            {status === "loading" ? (
      ...
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {t("dialog", "resultsFor")}{" "}
              <span className="font-mono">{getDialogTitle()}</span>
            </DialogTitle>
            <DialogDescription>
          ...
          <Tabs defaultValue="summary" className="w-full">
            ...
            <TabsContent value="fullText">
              {fullTextResult && (
                <Textarea
                  readOnly
                  value={fullTextResult.pages
                    .map(
                      (p) =>
                        `--- ${t("dialog", "page")} ${p.pageNumber} ---\n${
                          p.text
                        }`
                    )
                    .join("\n\n")}
