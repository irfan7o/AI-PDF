...
export default function PdfToImage() {
    ...
    const handleDownloadImage = (imageUri: string, index: number) => {
        ...
        link.href = imageUri;
        link.download = `${file?.name.replace('.pdf', '') || 'image'}-page-${index + 1}.jpg`;
        ...
        return (
            <Card className="w-full max-w-4xl shadow-sm rounded-xl">
                <CardHeader className="text-center">
                    <CardTitle className="font-headline text-2xl">{t('pdfToImage', 'successTitle')}</CardTitle>
                    <CardDescription>{t('pdfToImage', 'successDescription', { count: imageResult.images.length })}</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[50vh] overflow-y-auto p-1">
                        {imageResult.images.map((imageUri, index) => (
                            <div key={index} className="relative group aspect-[7/10] border rounded-lg overflow-hidden">
                                <Image src={imageUri} alt={`Page ${index + 1}`} layout="fill" objectFit="contain" />
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <Button variant="secondary" size="icon" className="h-10 w-10 rounded-full" onClick={() => handleDownloadImage(imageUri, index)}>
    ...
    return (
        <>
            <Card className="w-full max-w-lg shadow-sm rounded-xl">
                 <CardHeader className="text-center">
                     <CardTitle className="font-headline text-2xl">{t('floatingMenu', 'pdfToImage')}</CardTitle>
                     <CardDescription>{t('main', 'pdfToImageDescription')}</CardDescription>
                 </CardHeader>
                <CardContent
                    onDrop={handleDrop}
                ...
                >
                    <div className={cn(
                        ...
                        {(status === 'selected' || status === 'converting' || status === 'error') && file && (
                ...
                <CardFooter className="flex-col gap-4">
                    {status === 'converting' ? (
                        <div className="flex flex-col items-center gap-2">
                           ...
                           <p className="text-muted-foreground">{t('status', 'convertingToImage')}</p>
                        </div>
                    ) : (
