...
export default function ImageToPdf() {
  ...
  const processFiles = (filesToProcess: File[]) => {
    const validImageFiles = filesToProcess.filter((file) =>
    ...
    if (validImageFiles.length !== filesToProcess.length) {
      toast({
        ...
        description: t("toast", "invalidImageFileTypeDesc"),
      });
    }
      ...
      <Card className="w-full max-w-2xl shadow-sm rounded-xl">
        <CardHeader>
          <CardTitle className="font-headline text-2xl text-center">
            {t("floatingMenu", "imageToPdf")}
          </CardTitle>
          <CardDescription className="text-center">
            You have {images.length} image{images.length > 1 ? "s" : ""} ready
            to be converted. Drag to reorder, then convert.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
            ...
            >
              <div className="border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-muted-foreground hover:border-primary hover:text-primary transition-colors h-full w-full p-0">
                <FileUp className="h-8 w-8" />
                <p className="text-xs mt-2 text-center">
                  {t("uploadArea", "addMore")}
                </p>
              </div>
            </label>
            <input
    ...
    <>
      <Card className="w-full max-w-lg shadow-sm rounded-xl">
        ...
        >
          <div
            className={cn(
              "group w-full min-h-[300px] h-full rounded-lg border-2 border-dashed p-12 text-center transition-colors flex flex-col items-center justify-center cursor-pointer hover:border-primary hover:bg-primary/10"
            )}
          >
            {status === "idle" && (
              <div className="flex flex-col items-center justify-center h-full">
                ...
                <p className="mt-4 font-semibold text-foreground">
                  {t("uploadArea", "dragAndDropImages")}
                </p>
