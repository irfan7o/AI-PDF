...
export default function PdfToAudio() {
  ...
  useEffect(() => {
    // Initialize sample audio only on client side
    if (typeof window !== "undefined" && !sampleAudioRef.current) {
  ...
  const handlePlaySample = async (voiceId: string, voiceName: string) => {
    ...
    try {
      ...
      if (result.audioDataUri) {
        sampleAudio.src = result.audioDataUri;
        sampleAudio.play();
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            {voices.map((voice) => (
              <Label
                key={voice.id}
                className={cn(
                  "group cursor-pointer rounded-lg p-4 transition-colors hover:bg-primary hover:text-primary-foreground flex justify-between items-center",
                  selectedVoice === voice.id ? "bg-primary/10" : "bg-muted"
                )}
                onClick={() => {
      ...
      <Card className="w-full max-w-lg shadow-sm rounded-xl">
        <CardHeader className="text-center">
          <CardTitle className="font-headline text-2xl">
            {t("floatingMenu", "pdfToAudio")}
          </CardTitle>
          <CardDescription>
            {t("main", "pdfToAudioDescription")}
          </CardDescription>
        </CardHeader>
        <CardContent
          onDrop={handleDrop}
